...
def validStiff3DP1(**kwargs):
  ...
  tstart=time.time()
  MOptV2=StiffAssembling3DP1OptV2(Th.nq,Th.nme,qT,meT,Th.volumes)
  ...
  for i in range(len(LF)):
    test=TestStiff3D(LF[i][0],LF[i][1])
  ...
  Error=numpy.zeros(n)
  k=len(LF)-1
  test=TestStiff3D(LF[k][0],LF[k][1])
  ...
  for i in range(n):
    ...
    if Verbose: 
      ...
      print("        StiffAssembling3DP1OptV2 CPU times : %3.3f(s)" % TT)
    U=test.fu(Th.q[0],Th.q[1],Th.q[2])
    V=test.fv(Th.q[0],Th.q[1],Th.q[2])
  ...
  if checkTest3(h,Error)==1:
    sys.exit() 
  if Plot:        
    PlotTest3(h,Error,'Test 3 : Stiffness Matrix (3D/P1)') 
